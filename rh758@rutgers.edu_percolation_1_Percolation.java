...
public class Percolation {
	...
	*/
	public void openSite (int row, int col) {
		...
		int temp = (row* gridSize + col);

		if (row ==0)
		{
			wquFind.union(virtualTop, temp);
		}
		else if (row== gridSize-1)
		{
			wquFind.union(virtualBottom, temp);
		}
		if(col > 0 && grid[row][col-1])
		{
			wquFind.union(temp,temp-1);
		}
		if (row > 0 && grid[row -1][col])
		{
			wquFind.union(temp, temp- gridSize);
		}
	...
	*/
	public boolean percolationCheck () {
		...
		boolean temp = (wquFind.find(virtualBottom) == wquFind.find(virtualTop));
	...
	public void openAllSites (double probability, long seed) {
		...
		for(int a =0; a<gridSize; a+=1)
		{
			for (int b=0; b< gridSize; b+=1)
			{
